...
class ControlTempsScreenlet (screenlets.Screenlet):
	...
	def __init__ (self, **keyword_args):
		...
		#HoraApagar = "15:00"
		TempsTarja = timedelta(minutes= server.AddLogLloc(self.IdUser,strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())),'ENTRA',config.getElementsByTagName('WebServer')[0].attributes["Lloc"].value))
	...
	def update (self):
		...
		if segons == 0:
			...
			minuts = minuts - 1
                        ...
                        server.AddLogLloc(self.IdUser,strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())),'VIU',config.getElementsByTagName('WebServer')[0].attributes["Lloc"].value)
			logging.debug("Enviat Temps: Status:VIU")
			if  minuts == 0:
                                hora = hora - 1
                                
                                if str(hora)+':'+str(minut) == '0:05':
                                        os.system('gxmessage "Falten 5 minuts" &')
                                if str(hora)+':'+str(minut) == '0:00':
                                        os.system('gxmessage "S\'ha acabat el temps" &')
                                        server.AddLogLloc(self.IdUser,strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())),'SURT',config.getElementsByTagName('WebServer')[0].attributes["Lloc"].value)
                #			os.system("gnome-session-save --kill --force-logout")
                                minuts = 59
