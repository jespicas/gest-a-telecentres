...
class ControlTempsScreenlet (screenlets.Screenlet):
	...
	tmHoraApagar = timedelta(hours=0)
	config = minidom.parse(unicode(os.getenv("HOME")+"/.screenlets/ControlTemps/",sys.getfilesystemencoding())+'/config.xml')
	skip_taskbar = 'True'
	...
	def __init__ (self, **keyword_args):
		...
		self.skip_taskbar = True
		config = minidom.parse(unicode(os.getenv("HOME")+"/.screenlets/ControlTemps/",sys.getfilesystemencoding())+'/config.xml')
		servidorSoap = config.getElementsByTagName('WebServer')[0].attributes["IP"].value
		logging.debug("servidorSOAP"+servidorSoap)
		server = SOAPProxy('http://'+servidorSoap+':'+config.getElementsByTagName('WebServer')[0].attributes["Port"].value+'/')
...
#				if timedelta(hours= int(horaS.split(':')[0]), minutes=int(horaS.split(':')[1])) < timedelta(hours= int(HoraApagar.split(':')[0]), minutes=int(HoraApagar.split(':')[1])):
		...
		logging.debug(str(os.environ['USER']))
		#self.IdUser = server.GetUser(8636071700716196)
		logging.debug("HoraApagar:"+HoraApagar)
		logging.debug("HoraActual:"+HoraActual)
		#HoraApagar = "15:00"
		TempsTarja = timedelta(minutes= server.AddLog(self.IdUser,strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())),'ENTRA'))
		...
		logging.debug("HoraApagar"+str(tmHoraApagar))
		tmHoraActual = timedelta(hours=int(HoraActual.split(':')[0]),minutes=int(HoraActual.split(':')[1]))
		logging.debug("HoraActual"+str(tmHoraActual))
		horaDescompatarProva = tmHoraApagar - tmHoraActual
		logging.debug("HoraApagar - HoraActual:"+str(horaDescompatarProva))
		if TempsTarja > (tmHoraApagar - tmHoraActual):
			horaDescomptar = (tmHoraApagar - tmHoraActual)
		else:
			horaDescomptar =  TempsTarja 
		...
		try:
			...
			logging.debug(' No trobat Applicacio Gestio Clietns')
			pass
		...
		if hora+':'+minut == '0:00':
			...
			server.AddLog(self.IdUser,strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())),'SURT')
			os.system('gxmessage "Ara huriem de posar apagar')
			logging.debug("gnome-session kill")
	...
	def update (self):
		...
		if segons == 0:
			...
			minuts = minuts - 1
                        ...
                        server.AddLog(self.IdUser,strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())),'VIU')
			logging.debug("Enviat Temps: Status:VIU")
			if  minuts == 0:
                                hora = hora - 1
                                
                                if str(hora)+':'+str(minuts) == '0:05':
...
class Server(Thread):
	...
	def run(self):
		...
		d = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
		eth = str(config.getElementsByTagName('Ethernet')[0].attributes["Nom"].value)
...
if __name__ == "__main__":
	...
	logging.basicConfig(level=logging.DEBUG,format = '%(asctime)s %(levelname)s %(message)s',datefmt = '%a, %d %b %Y %H:%M%S',filename='.screenlets/ControlTemps/ControlTemps.log',filemode='w' )
